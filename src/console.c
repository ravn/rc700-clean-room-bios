...
static void set_cursor(console_t *con, uint8_t col, uint8_t row) {
    con->curx = col;
    con->cursy = row;
    con->cury = (uint16_t)row * SCREEN_COLS;
    con->cursor_dirty = 1;
}
...
static void set_bgmap_bit(console_t *con, uint8_t col, uint8_t row) {
    uint16_t bit_index = (uint16_t)row * SCREEN_COLS + col;
    con->bgmap[bit_index >> 3] |= (uint8_t)(0x80 >> (bit_index & 7));
}
...
void console_cursor_left(console_t *con) {
    ...
    con->cury = (uint16_t)con->cursy * SCREEN_COLS;
    con->cursor_dirty = 1;
}
...
void console_erase_to_eol(console_t *con) {
    uint16_t pos = con->cury + con->curx;
    uint16_t end = con->cury + SCREEN_COLS;
...
void console_insert_line(console_t *con) {
    ...
    uint16_t src = (uint16_t)(SCREEN_ROWS - 2) * SCREEN_COLS;
    uint16_t dst = (uint16_t)(SCREEN_ROWS - 1) * SCREEN_COLS;
    uint16_t cursor_row_offset = con->cury;

    for (uint16_t s = src; s >= cursor_row_offset && s <= src; s -= SCREEN_COLS) {
    ...
    /* Scroll background bitmap if active */
    if (con->bg_flag != BG_OFF) {
        uint8_t bg_row = con->cursy;
        uint16_t bg_src = (uint16_t)(SCREEN_ROWS - 2) * BGMAP_BYTES_PER_ROW;
        uint16_t bg_cursor = (uint16_t)bg_row * BGMAP_BYTES_PER_ROW;
        for (uint16_t s = bg_src; s >= bg_cursor && s <= bg_src; s -= BGMAP_BYTES_PER_ROW) {
...
void console_delete_line(console_t *con) {
    ...
    uint16_t cursor_row_offset = con->cury;
    uint16_t last_row = (uint16_t)(SCREEN_ROWS - 1) * SCREEN_COLS;

    for (uint16_t d = cursor_row_offset; d < last_row; d += SCREEN_COLS) {
    ...
    if (con->bg_flag != BG_OFF) {
        uint8_t bg_row = con->cursy;
        uint16_t bg_cursor = (uint16_t)bg_row * BGMAP_BYTES_PER_ROW;
        uint16_t bg_last = (uint16_t)(SCREEN_ROWS - 1) * BGMAP_BYTES_PER_ROW;
        for (uint16_t d = bg_cursor; d < bg_last; d += BGMAP_BYTES_PER_ROW) {
...
static void process_xy(console_t *con, uint8_t byte) {
    uint8_t coord = (byte & 0x7F) - 32;
...
static void put_printable(console_t *con, uint8_t ch) {
    uint16_t pos = con->cury + con->curx;
    con->display[pos] = ch;
